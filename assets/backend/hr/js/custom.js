...
$(document).ready(function () {
	...
	if ($('#taskList_category').length)  {
        taskList_category = $('#taskList_category').DataTable({
           "paging": true,
            "lengthMenu": [10, 20, 50, 100, 200, 500, 1000],
            "lengthChange": true,
            "language": {
            ...
            // "serverSide": true,
            "ajax": {                
                url: base_url+"hr/admin/get-task-category", 
                type: "post", 
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    if (parseInt(XMLHttpRequest.status) == 419) {
                        alert("You are logged out. Please login.");
                    ...
                    $("#taskList_category tbody").append('<tr><td colspan="7" class="text-center">No records found</td></tr>');
                    $("#taskList_category_processing").css("display", "none");
                }
            }            
        });
    }
	...
	if ($('#taskList').length)  {
        taskList = $('#taskList').DataTable({
           "paging": true,
            ...
            // "serverSide": true,
            "ajax": {                
			url: base_url+"hr/admin/get-task-list", 
                type: "post", 
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    if (parseInt(XMLHttpRequest.status) == 419) {
                        alert("You are logged out. Please login.");
                    ...
                    $("#taskList tbody").append('<tr><td colspan="7" class="text-center">No records found</td></tr>');
                    $("#taskList_processing").css("display", "none");
                }
            }            
        });
    }
