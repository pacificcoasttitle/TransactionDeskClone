...
/*	---------------------------------------------------------------------------
	...
	$LoanAmount_1 = isset($_POST["LoanAmount1"]) && !empty($_POST["LoanAmount1"]) ? strip_tags(trim($_POST["LoanAmount1"])) : '';

	$LoanAmount_2 = isset($_POST["LoanAmount2"]) && !empty($_POST["LoanAmount2"]) ? strip_tags(trim($_POST["LoanAmount2"])) : '';

	$Refinance = isset($_POST["Refinance"]) && !empty($_POST["Refinance"]) ? 1 : 0;

	$Purchase = isset($_POST["Purchase"]) && !empty($_POST["Purchase"]) ? 1 : 0;

	$Commercial = isset($_POST["Commercial"]) && !empty($_POST["Commercial"]) ? 1 : 0;
	...
	else
	{
		...
		if($conn)
		{
			if(empty($_POST['id']))
			{
				...
				$sql = "INSERT INTO `pctorder`.`customer_basic_details` (`customer_number`,`first_name`,`last_name`,`telephone_no`,`email_address`,`company_name`,`street_address`,`city`,`zip_code`,`status`,`created_at`) 
				VALUES
				('".$customer_number."', '".$OpenName."','".$OpenLastName."','".$Opentelephone."','".$OpenEmail."','".$CompanyName."','".$StreetAddress."','".$City."','".$Zipcode."','1','".$time."'
			    );";
				...
				if ($conn->query($sql) === TRUE) 
				{
					...
					if($id)
					{
						...
						$transaction_query = "INSERT INTO `pctorder`.`transaction_details` (`customer_id`,`sales_representative`,`title_officer`,`sales_loan_amount_1`,`sales_loan_amount_2`,`sales_loan_amount_3`,`is_refinance`,`is_purchase`,`is_commercial`,`is_ccr`,`is_underlying_docs`,`is_plotted_easements`,`status`,`created_at`)
							VALUES
							('".$id."','".$SalesRep."','".$TitleOfficer."','".$LoanAmount."','".$LoanAmount_1."','".$LoanAmount_2."','".$Refinance."','".$Purchase."','".$Commercial."','".$CCR."','".$Docs."','".$Ease."','1','".date("Y-m-d H:i:s")."'
							);";
			...
			else
			{
				...
				$transaction_query = "INSERT INTO `pctorder`.`transaction_details` (`customer_id`,`sales_representative`,`title_officer`,`sales_loan_amount_1`,`sales_loan_amount_2`,`sales_loan_amount_3`,`is_refinance`,`is_purchase`,`is_commercial`,`is_ccr`,`is_underlying_docs`,`is_plotted_easements`,`status`,`created_at`)
				VALUES
				('".$customer_id."','".$SalesRep."','".$TitleOfficer."','".$LoanAmount."','".$LoanAmount_1."','".$LoanAmount_2."','".$Refinance."','".$Purchase."','".$Commercial."','".$CCR."','".$Docs."','".$Ease."','1','".date("Y-m-d H:i:s")."'
				);";
		...
		$redirectForm_url ='http://localhost/beta-pct-calc/order/order-submitted.php';
