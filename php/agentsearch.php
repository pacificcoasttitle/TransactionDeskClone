<?php
    include dirname(__FILE__).'/../config/database.php';

    // Get search term 
    $searchTerm = $_GET['term'];

    // Fetch matched data from the database 
    // $query = $conn->query("SELECT *, CONCAT(first_name, ' ',last_name, ' - ',email_address) AS value FROM agents WHERE first_name LIKE '%".$searchTerm."%' AND status = 1 ORDER BY first_name ASC");
    $query = $conn->query("SELECT *, CONCAT(name,' - ',email_address) AS value FROM agents WHERE name LIKE '%".$searchTerm."%' AND status = 1 ORDER BY name ASC");

    $agentInfo = array(); 
    if($query->num_rows > 0)
    { 
        while($row = $query->fetch_assoc())
        {
            $data['id'] = isset($row['id']) && !empty($row['id']) ? $row['id'] : '';
            
            $data['value'] = isset($row['value']) && !empty($row['value']) ? $row['value'] : '';

            $data['name'] = isset($row['name']) && !empty($row['name']) ? $row['name'] : '';
            /* $data['last_name'] = isset($row['last_name']) && !empty($row['last_name']) ? $row['last_name'] : '';*/
            $data['email_address'] = isset($row['email_address']) && !empty($row['email_address']) ? $row['email_address'] : '';
            $data['telephone_no'] = isset($row['telephone_no']) && !empty($row['telephone_no']) ? $row['telephone_no'] : '';
            $data['company'] = isset($row['company']) && !empty($row['company']) ? $row['company'] : '';
            array_push($agentInfo, $data); 
        } 
    } 

    // Return results as json encoded array 
    echo json_encode($agentInfo); 
?>