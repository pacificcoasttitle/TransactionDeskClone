<?php
    ...
    $query = $conn->query("SELECT *, CONCAT(first_name, ' ',last_name, ' - ',email_address) AS value, CONCAT(first_name, ' ',last_name) AS full_name FROM customer_basic_details WHERE first_name LIKE '%".$searchTerm."%' AND status = 1 AND is_escrow = 1 ORDER BY first_name ASC");
