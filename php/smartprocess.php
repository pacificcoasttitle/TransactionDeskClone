...
/*	---------------------------------------------------------------------------
	...
	$BuyerAgentId = isset($_POST["BuyerAgentId"]) && !empty($_POST["BuyerAgentId"]) ? strip_tags(trim($_POST["BuyerAgentId"])) : '';

	$BuyerAgentEmailAddress = isset($_POST["BuyerAgentEmailAddress"]) && !empty($_POST["BuyerAgentEmailAddress"]) ? strip_tags(trim($_POST["BuyerAgentEmailAddress"])) : '';
	...
	$ListingAgentId = isset($_POST["ListingAgentId"]) && !empty($_POST["ListingAgentId"]) ? strip_tags(trim($_POST["ListingAgentId"])) : '';
	...
	else
	{
		...
		{*/
			...
			if(isset($result) && !empty($result))
			{
				...
				if($orderNumber)
				{
					...
					include dirname(__FILE__).'/templates/orderdetails.php';
		...
		if($conn)
		{
			...
			if(empty($_POST['id']))
			{
				...
				$sql = "INSERT INTO `pctorder`.`customer_basic_details` (`customer_number`,`first_name`,`last_name`,`telephone_no`,`email_address`,`password`,`company_name`,`street_address`,`city`,`zip_code`,`is_escrow`,`status`,`created_at`) 
				VALUES
				('".$customer_number."', '".$OpenName."','".$OpenLastName."','".$Opentelephone."','".$OpenEmail."','".$password."','".$CompanyName."','".$StreetAddress."','".$City."','".$Zipcode."','1','1','".$time."'
			    );";
				...
				if ($conn->query($sql) === TRUE) 
				{
					...
					if($id)
					{
						...
						$property_query = "INSERT INTO `pctorder`.`property_details` (`customer_id`,`buyer_agent_id`,`listing_agent_id`,`escrow_lender_id`,`address`,`full_address`,`apn`,`county`,`legal_description`,`primary_owner`,`secondary_owner`,`additional_details`,`status`,`created_at`) 
						VALUES
						('".$id."','".$BuyerAgentId."','".$ListingAgentId."','".$EscrowLenderId."','".$PropertyAddress."','".$FullProperty."','".$apn."','".$County."','".$LegalDescription."','".$PrimaryOwner."','".$SecondaryOwner."','".$sendermessage."','1','".date("Y-m-d H:i:s")."'
						);";
			...
			else
			{
				...
				$property_query = "INSERT INTO `pctorder`.`property_details` (`customer_id`,`buyer_agent_id`,`listing_agent_id`,`escrow_lender_id`,`address`,`full_address`,`apn`,`county`,`legal_description`,`primary_owner`,`secondary_owner`,`additional_details`,`status`,`created_at`) 
				VALUES
				('".$customer_id."','".$BuyerAgentId."','".$ListingAgentId."','".$EscrowLenderId."','".$PropertyAddress."','".$FullProperty."','".$apn."','".$County."','".$LegalDescription."','".$PrimaryOwner."','".$SecondaryOwner."','".$sendermessage."','1','".date("Y-m-d H:i:s")."'
				);";
