...
/*	---------------------------------------------------------------------------
	...
	$Owntype = strip_tags(trim($_POST["Owntype"]));
	$Owner = strip_tags(trim($_POST["Owner"]));
	$Nonowner = strip_tags(trim($_POST["Nonowner"]));
		...
		if ($_FILES['orderfiles']['error'] == 0) {
			...
			/*	---------------------------------------------------------------------
				...
				if($redirectForm == true){
			  	...
			  	echo '<div class="alert notification alert-success">Your farm request is being submitted. Please wait for confirmation.</div>'; 
