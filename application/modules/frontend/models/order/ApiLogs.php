...
class ApiLogs extends CI_Model 
{
    ...
    public function syncPrelimLogs($user_id, $api_type, $request_type, $request_url, $request_data, $response_data, $order_id = 0, $logId = 0) 
    {
        if(is_array($request_data)) {
            $request_data = json_encode($request_data, true);
        }
        if ($logId == 0) {
            ...
            $this->db->insert('pct_order_prelim_logs', $data);
            return $this->db->insert_id();
        } else {
            if (is_array($response_data)) {
                $response_data = json_encode($response_data, true);
            ...
            $this->db->update('pct_order_prelim_logs', $data, array('id' => $logId));
        }
    }
