...
class Home_model extends CI_Model
{
    ...
    public function get_sp_customers($params = array(), $is_master_search = 0)
    {
        ...
        //         if ($key == 'status') {
        //             $this->db->where('pct_softpro_lookup_table.status', $val);
        //         } else {
        //             $this->db->where($key, $val);
        //         }
        //     }
    ...
    public function get_sp_companies($params = array(), $is_master_search = 0)
    {
        $table = 'sp_company'; //$this->table;

        $this->db->select('sp_company.*');
        $this->db->from($table);
        ...
        if (array_key_exists("returnType", $params) && $params['returnType'] == 'count') {
            ...
            if (array_key_exists("id", $params)) {
                ...
                if (array_key_exists("start", $params) && array_key_exists("limit", $params)) {
                    ...
                    $this->db->select("CONCAT(name, ' - ',CONCAT_WS(',', address1, city, state, zip)) AS value, name AS full_name");
                    $this->db->group_start()
                ...
                if (array_key_exists("is_escrow_company", $params) && $params['is_escrow_company'] == 1) {
                    $this->db->where('is_escrow_company', 1);
                }
                
                if (array_key_exists("is_lender", $params) && $params['is_lender'] == 1) {
                    $this->db->where('is_lender', 1);
                }

                if (array_key_exists("is_mortgage_broker", $params) && $params['is_mortgage_broker'] == 1) {
                    $this->db->where('is_mortgage_broker', 1);
                }

                if (array_key_exists("is_selling_agent", $params) && $params['is_selling_agent'] == 1) {
    ...
    public function get_sp_company($params = [])
    {
        $table = 'sp_company';
    ...
    public function get_product_types($params = array())
    {
        $table = 'pct_softpro_product_type';
    ...
    public function get_order_types($params = array())
    {
        $table = 'pct_softpro_order_type';
