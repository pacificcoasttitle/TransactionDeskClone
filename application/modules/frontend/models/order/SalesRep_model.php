...
class SalesRep_model extends CI_Model 
{
    public function getSummaryDetailsForSalesRep($userId)
    {
        ...
        $this->db->where('pct_softpro_lookup_table.email_address != ""');
        ...
        $this->db->join('pct_softpro_lookup_table', 'pct_softpro_lookup_table.id = transaction_details.sales_representative','inner');
        $this->db->join('pct_softpro_lookup_table as user_details', 'user_details.id = order_details.customer_id','left');
