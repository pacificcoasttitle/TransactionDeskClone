		...
		$("#edit_LenderCompany" ).focusin(function() {
	    	...
	    	else
	    	{
	    		...
	    		/*$("#edit_LenderCompany").autocomplete({
					...
					select: function( event, ui ) {
						event.preventDefault();
						$("#edit_LenderCompany").val(ui.item.partner_name);
					
						if(ui.item.address1) {
							$("#edit_LenderAddress").val(ui.item.address1).parent().addClass('state-success');
						} else {
							$("#edit_LenderAddress").val('').parent().removeClass('state-success').addClass('state-error');
						}

						if(ui.item.city) {
							$("#edit_LenderCity").val(ui.item.city).parent().addClass('state-success');
						} else {
							$("#edit_LenderCity").val('').parent().removeClass('state-success').addClass('state-error');
						}
							
						if(ui.item.zip) {
							$("#edit_LenderZipcode").val(ui.item.zip).parent().addClass('state-success');
						} else {
							$("#edit_LenderZipcode").val('').parent().removeClass('state-success').addClass('state-error');
						}
						$("#edit_LenderId").val('');
						$("#edit_state").val(ui.item.state);
						$("#edit_partner_id").val(ui.item.partner_id);
					},
					change: function( event, ui ) {
						if (ui.item == null) {
							$("#LenderCompany").parent().removeClass('state-success').addClass('state-error');
