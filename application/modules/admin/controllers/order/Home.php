...
class Home extends MX_Controller
{
    ...
    public function get_cron_logs()
    {
        ...
        if (isset($_POST['draw']) && !empty($_POST['draw'])) {
            ...
            $params['is_escrow']   = 0;
            $pageno                = ($params['start'] / $params['length']) + 1;
            $admin_logs_list       = $this->home_model->get_cron_logs($params);
            $json_data['draw']     = intval($params['draw']);
        } else {
            $params['searchvalue'] = isset($_POST['keyword']) && !empty($_POST['keyword']) ? $_POST['keyword'] : '';
            $admin_logs_list       = $this->home_model->get_cron_logs($params);
    ...
    public function get_sp_agent_list()
    {
        ...
        if(isset($agent_lists['data']) && !empty($agent_lists['data']))
        {
            foreach ($agent_lists['data'] as $key => $value) 
            {
                ...
                if(isset($_POST['draw']) && !empty($_POST['draw']))
                {
                    $editUserUrl = base_url().'order/admin/edit-agent/'.$value['id'];
                    $action = "<div style='display: flex;justify-content: space-evenly;'><a href='".$editUserUrl."' class='edit-agent' title ='Edit Agent Detail'><i class='fas fa-edit' aria-hidden='true'></i></a>";
    ...
    public function get_sp_escrow_list()
    {
        ...
        if (isset($escrowLists['data']) && !empty($escrowLists['data'])) {
            foreach ($escrowLists['data'] as $key => $value) {
                ...
                if (isset($_POST['draw']) && !empty($_POST['draw'])) {
                    $editUserUrl = base_url().'order/admin/edit-user/'.$value['id'];
                    $action = "<div style='display: flex;justify-content: space-evenly;'><a href='".$editUserUrl."' class='edit-agent' title ='Edit Escrow Detail'><i class='fas fa-edit' aria-hidden='true'></i></a>";
                    // $action = "<a href='javascript:void(0);' class='btn btn-action edit-group' data-id=".$value->id." data-name='".$value->name."' title ='Edit Group Detail'><span class='fas fa-edit' aria-hidden='true'></span></a>";

                    $action       .= "<a href='javascript:void(0);' onclick='deleteCustomer(" . $value['id'] . ")'  title='Delete Customer'><span class='fas fa-trash' aria-hidden='true'></span></a></div>";
                    $nestedData[] = $action;
                }
    ...
    public function editUser()
    {
        ...
        $id = $this->uri->segment(4);        
        $data = array();
        $data['title'] = 'PCT Order: Edit User';
        if(isset($id) && !empty($id))
        {
        ...
        if (isset($lender_lists['data']) && !empty($lender_lists['data'])) {
            foreach ($lender_lists['data'] as $key => $value) {
                ...
                if (isset($_POST['draw']) && !empty($_POST['draw'])) {
                    $editUserUrl = base_url().'order/admin/edit-user/'.$value['id'];
                    $action = "<div style='display: flex;justify-content: space-evenly;'><a href='".$editUserUrl."' class='edit-agent' title ='Edit Escrow Detail'><i class='fas fa-edit' aria-hidden='true'></i></a>";
                    $action .= "<a href='javascript:void(0);' onclick='deleteCustomer(" . $value['id'] . ")' title='Delete Customer'><i class='fas fa-trash' aria-hidden='true'></i></a></div>";
                    $nestedData[] = $action;
                }
        ...
        if (isset($mortgage_lists['data']) && !empty($mortgage_lists['data'])) {
            foreach ($mortgage_lists['data'] as $key => $value) {
                ...
                if (isset($_POST['draw']) && !empty($_POST['draw'])) {
                    $editUserUrl = base_url().'order/admin/edit-user/'.$value['id'];
                    $action = "<div style='display: flex;justify-content: space-evenly;'><a href='".$editUserUrl."' class='edit-agent' title ='Edit Escrow Detail'><i class='fas fa-edit' aria-hidden='true'></i></a>";
                    $action .= "<a href='javascript:void(0);' onclick='deleteCustomer(" . $value['id'] . ")' class=''  title='Delete Customer'><span class='fas fa-trash' aria-hidden='true'></span></a></div>";
                    $nestedData[] = $action;
                }
                $data[] = $nestedData;
            }
