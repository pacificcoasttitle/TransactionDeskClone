...
class Orders extends MX_Controller {
    ...
    public function index()
    {
        ...
        $this->template->addJS( base_url('assets/backend/escrow/js/orders.js?v=orders_'.$this->orders_js_version) );
        $this->template->show("escrow", "orders", $data);
    }
    ...
    public function getOrders()
    {
	    ...
	    if (isset($userTypes['data']) && !empty($userTypes['data'])) {
	    	foreach ($userTypes['data'] as $key => $value)  {
                ...
                $editUrl = base_url().'escrow/admin/order-tasks/'.$value['id'];
    ...
    public function orderTasks($id)
	{
        ...
        if ($this->input->post()) {
            ...
            if ($userdata['is_escrow_officer'] == 1) {
                $managerInfo = $this->escrow_user_model->get_by(array('department_id' => 4, 'user_type_id' => 4));
                $notificationData = array(
                    'sent_user_id' => $managerInfo->id,
                    'message' => $message,
                    'type' =>  'completed'
                );
        ...
        $this->template->show("escrow", "order_tasks", $data);
	}
    ...
    public function markAsRead()
    {
        ...
        $this->common->update($data, $condition, 'pct_escrow_notifications');
        $response = array(
            'success' => 'true',
