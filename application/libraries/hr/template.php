...
class Template
{
    ...
    public function __construct()
    {
        ...
        $this->addJS( base_url('assets/backend/vendor/datatables/jquery.dataTables.js') );
        $this->addJS( base_url('assets/backend/vendor/datatables/dataTables.bootstrap4.js') );
        $this->addJS( base_url('assets/backend/vendor/datatables/dataTables.buttons.min.js') );
    ...
    private function load_JS_and_css()
    {
        ...
        if ( $this->js_file ) {
            foreach( $this->js_file as $js ) {
                $this->data['foot_script'] .= "<script type='text/javascript' src=".$js->file."></script>". "\n";
            }
        }
    }
}