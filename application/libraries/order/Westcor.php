...
class Westcor
{
    ...
    public function generateCplDocument($orderId, $orderDetails)
    {
        $userdata = $this->CI->session->userdata('user');
        if (empty($userdata)) {
            $userdata['id'] = 0;
        ...
        if (!empty($lenderDetails)) {
            $lenders[] = array(
                ...
                'tvid' => 0,
                'name' => !empty($lenderFormData) ? $lenderFormData['company_name'] : $lenderDetails['name'],
                ...
                'zip' => !empty($lenderFormData) ? $lenderFormData['zip'] : $lenderDetails['zip'],
                'address' => !empty($lenderFormData) ? $lenderFormData['address1'] : $lenderDetails['address1'],
                ...
                if ($resultResCPL['messages']['error']) {
                    foreach ($resultResCPL['messages']['error'] as $error) {
                        if (empty($orderDetails['westcor_lender_id']) && strtolower($error) == 'lender could not be found.') {
                            ...
                            $this->CI->home_model->update($order_details, $condition, 'order_details');
                            redirect(base_url() . "create-cpl/" . $orderId);
                        }
