        ...
        jQuery.fn.DataTable.Api.register('buttons.exportData()', function (options) {
            ...
            if (this.context.length) 
            {
                if(this.context[0].sTableId == 'tbl-customers-listing')
                {
                    var jsonResult = $.ajax({
                        type: "POST",
                        url: base_url+"?home/get_customer_list",
                        data: {
                ...
                else
                {
                    var jsonResult = $.ajax({
                        type: "POST",
                        url: base_url+"?agent/get_agent_list",
                        data: {
    ...
    if ($('#tbl-customers-listing').length) 
    {
        customer_list = $('#tbl-customers-listing').DataTable({
           ...
           "paging": true,
            ...
            "serverSide": true,
            "ajax": {                
                url: base_url+"?home/get_customer_list", // json datasource
    ...
    if ($('#tbl-agents-listing').length) 
    {
        agent_list = $('#tbl-agents-listing').DataTable({
           ...
           "paging": true,
            ...
            "serverSide": true,
            "ajax": {                
                url: base_url+"?agent/get_agent_list", // json datasource
                type: "post", // method  , by default get
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    if (parseInt(XMLHttpRequest.status) == 419) {
                        alert("You are logged out. Please login.");
...
function deleteCustomer(id)
{
    ...
    if (ready) {
        $.ajax({
            url: base_url+"?home/delete_customer",
            method: "POST",
            data : {id:id},
            success: function(data){
