    ...
    <div class="section-title-page5 area-bg area-bg_blue area-bg_op_90 parallax">
    ...
    <section class="content-wrapper">
        <div class="row">
            <div class="twelve column">
                <div class="panel">
                    <p>The charges quoted on this web site are estimates only, and should not be relied on as accurately reflecting the charges for a specific transaction. The actual charges may vary, depending on the availability of discounts, requests for special coverages or services, or other matters specific to the transaction. Please contact your local Pacific Coast Title office or agent for charges associated with a specific transaction. Contact information for Pacific Coast Title Company offices in your area is available at www.pct.com/branches</p>
                </div>
            </div>
        </div>
        <div class="row">
            ...
            <?php if($this->session->userdata('mpuserid') !=""){?>
                Hi, <?php echo $this->session->userdata('mpusername');?>  <a href="<?=base_url()?>index.php?welcome/logout">Logout</a>
            <?php } else{?>
               <a class="button orange create-repo-btn small" href="<?=base_url()?>index.php?welcome/signup"> Lender Partner Login</a>
               <?php } ?>
            </div>
            <div class="column six" >
            <form id="retrive_quote" action="<?=base_url()?>index.php?welcome/view_quote" method="POST" class="form-horizontal" role="form">
		...
		<div class="row">
			<form id="get_quote" action="<?=base_url()?>index.php?welcome/receipt" method="POST" role="form" >
			 
				 <div class="row mil">
				   <h2 class="ui-title-block-3 ui-title-block-3_sm" style="color:#888;" >Property Location</h2>
					<hr class="w">
					<div class="column four first">
					   <select type="text" placeholder="Select State" name="region" class="sarinput" id="repcounty" onchange="get_county_list(this.value)" >
					...
					<div class="column four">
						  <select type="text" placeholder="Select Rep" name="county" class="sarinput" onchange="get_city_list(this.value);" id="select_county" style="display:none">
					  <option value="" class="city_null">Select County</option>
								   
								   <?php foreach ($county_list as $county): ?>
										<option value="<?=$county->zone_name?>" class="region<?php $ss = str_replace(' ','_',$county->region); echo $ss;?>"><?=$county->zone_name?></option>
								   <?php endforeach ?>
								  </select>  
					</div>
					<div class="column four">
				...
				<div class="row" id="close_county_row" style="display:none;">
				 <h2 class="ui-title-block-3 ui-title-block-3_sm" style="color:#888;">Closing Property Location</h2>
				  ...
				  <div class="column three first">
						  <select type="text" placeholder="Select Rep" name="closing_zone" class="sarinput" onchange="get_close_city_list(this.value);" id="select_close_county" >
					  <option value="" class="city_null">Select County</option>
					...
					<div class="column three ">
						   <select type="text" placeholder="Select Rep" name="closing_county" class="sarinput" id="select_close_city" >
					  <option value="" class="city_null">Select City</option>
									 <?php foreach ($city_list as $county): ?>
										<option value="<?=$county->county_id_pk?>" class="zone<?php $ss = str_replace(' ','_',$county->zone_name); echo $ss;?>"><?=$county->county_name?></option>
								   <?php endforeach ?>
				...
				<div class="row">
				   <h2 class="ui-title-block-3 ui-title-block-3_sm" style="color:#888;">Rates Option</h2>
				   <hr class="w">
				...
				<!--	<div class="checkbox-inline">
					...
					<div class="clearfix">
						...
						<p>&nbsp;</p> 
				 <div class="row ">
				   <h2 class="ui-title-block-3 ui-title-block-3_sm" style="color:#888;">Transaction Details</h2>
					<hr class="w">
					<div class="column four first">
					   <select type="text" placeholder="Select Rep" name="transaction_type" class="sarinput" id="transacttype" onchange="change_transaction_type()">
					...
					<div class="column four">
						...
						<input type="text" placeholder="Sale Amount" id="saleamount"onkeypress="return isNumber(event,this)" name="transaction_amount" class="sarinput" autocomplete="off">
					<small style="color:#f33;display:none">Please enter correct value.</small>
					</div>
					<div class="column four">
						<span style='margin-left: -15px;'>$</span>
						<input type="text" placeholder="Loan Amount" id="loanamount"onkeypress="return isNumber(event,this)" name="loanamount" class="sarinput" autocomplete="off">
					<small style="color:#f33;display:none">Please enter correct value.</small>
			...
			<!--	  <div class="row">
				   ...
				   <h2 class="ui-title-block-3 ui-title-block-3_sm" style="color:#888;">Escrow/Settlement Service</h2>
					...
					<input type="reset" value="Reset Form" class="button  grey create-repo-btn" onclick="javascript:(location.reload());">
				  </div>
...
function get_city_list (zone_id) 
{
...
//       {
//            console.log('here1');
//        $('[name="title-rates"]').attr("checked","true");
//        $('[name="escrow-rates"]').attr("checked","true");
//        $('[name="endorsements"]').removeAttr("checked");
//        $('[name="recording-fees"]').attr("checked","true");
//       }
//       else if(sel == "Southern")
//       {
//       console.log('here2');
//        $('[name="title-rates"]').attr("checked","true");
//        $('[name="escrow-rates"]').removeAttr("checked");
//        $('[name="endorsements"]').removeAttr("checked");
//        $('[name="recording-fees"]').removeAttr("checked");
//      }
//        else if($('#all-fees').is(":checked"))
//        {
//
//            $('[name="title-rates"]').attr("checked","true");
//            $('[name="escrow-rates"]').attr("checked","true");
//            $('[name="endorsements"]').attr('checked',"true");
//            $('[name="recording-fees"]').attr('checked',"true");
//        }
//    }
//    else
...
function change_transaction_type () 
{
  ...
  $.ajax({
    url: '<?=base_url()?>index.php?welcome/endorsement_options/'+selected_value,
  })
  .done(function(data) {
    console.log("success");
  ...
  .always(function() {
   ...
   {	
       ...
       $('[name="eight"]').addClass('default');
		...
		$('#reppolicy').html("");
		$('#reppolicy').html('<option value="">Policy Type</option><option value="Regular">Alta Homeowners Policy </option>');
...
$('#lenpolicy').parent().show();
   ...
   else if(selected_value == "Re-Finance")
   {
    ...
    $('#reppolicy').html('<option value="">Policy Type</option><option selected="true" value="Residential">ALTA Residential Loan Policy</option>');
  }
...
function isNumber(evt,box)
                      {
                                ...
                                if (charCode > 31 && (charCode < 48 || charCode > 57)) {
                                    ...
                                    setTimeout(function(){$(box).siblings('small').hide();},1000);
                                    return false;
                                }
                                return true;
                      }

//document.getElementById("saleamount").onblur =function (){ 
//       if(this.value !="")
//       {
